...
export const deleteCakeById = async (req, res) => {
    ...
    try {
        ...
        if (!req.user.role == "ADMIT" || req.user.role == cake.ownerId)
            return res.status(403).send(" אין לך הרשאה למחוק")

        let deleted = await CakeModel.findByIdAndDelete(id)
        if (!deleted)
...
export const postCake = async (req, res) => {
    ...
    try {
        ...
        let newCake = new CakeModel({ name, price, baker, ExpiryDate, isMilk,ownerId:req.user._id });
        await newCake.save();
        res.json(newCake)
    }
    catch (err) {
